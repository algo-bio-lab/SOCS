...
def fugw_cost(pi, gamma, a, dx, b, dy, eps, rho, rho2,c,alpha):
    """Computes the full (U)GW functional with entropic term and KL
    penalty of marginals.
...
def init_plan_2d(a, b1, b2, init=None):
    """Initialize the plan if None is given, otherwise use the input plan
    ...
    if init is not None:
        ...
        return (
                a[:, None]
                * b1[None, :]
                / (a.sum() * b1.sum()).sqrt()+a[:,None]*b2[None,:]/(a.sum()*b2.sum()).sqrt()
        )
...
def compute_local_cost_fx(pi, a, dx, b, dy, dc, alpha, eps, rho, rho2, gdw, complete_cost=True):
    """Compute the local cost by averaging the distortion with the current
    transport plan.
    ...
    lcost = (1-alpha)*(distxx[:, None] + distyy[None, :] - 2 * distxy) + eps * kl_pi
    #lcost = (1-alpha)*lcost
    for x in range(lcost.shape[0]):
        lcost[:,x] = torch.mul(lcost[:,x],gdw)
    if rho < float("Inf"):
        lcost = (
                lcost
                + rho
...
def compute_local_cost_2d(pi, a, dx, b1, b2, dy, dc, alpha, eps, rho, rho2, complete_cost=True):
    distxy = torch.einsum(
        "ij,kj->ik", dx, torch.einsum("kl,jl->kj", dy, pi)
    )
    kl_pi = torch.sum(
        pi * (pi / ((a[:, None] * b1[None, :])+(a[:, None] * b2[None, :])) + 1e-10).log()
    )
    if not complete_cost:
        return - 2 * distxy + eps * kl_pi
    ...
    if rho2 < float("Inf"):
        lcost = (
                lcost
                + rho2
                * torch.sum(nu * (nu / b1 + 1e-10).log())
        )
        lcost = (
                lcost
                + rho2
                * torch.sum(nu * (nu / b2 + 1e-10).log())
        )
    lcost = lcost + (alpha/2)*dc
    return lcost
...
def log_sinkhorn_2d(lcost, f, g, a, b1, b2, mass, eps, rho, rho2, nits_sinkhorn,
                 tol_sinkhorn):
    if f is None or g is None:
        f, g = torch.zeros_like(a), torch.zeros_like(b2)
    f, g = log_translate_potential(f, g, lcost, a, b2, mass, eps, rho, rho2)

    # perform Sinkhorn algorithm in LSE form
    s_x1, s_y1 = aprox_softmin(lcost, a, b1, mass, eps, rho, rho2)
    s_x2, s_y2 = aprox_softmin(lcost, a, b2, mass, eps, rho, rho2)
    for j in range(nits_sinkhorn):
        f_prev = f.clone()
        g = s_x1(f)
        f = s_y1(g)
...
def exp_sinkhorn(ecost, u, v, a, b, mass, eps, rho, rho2, nits_sinkhorn,
    ...
    for j in range(nits_sinkhorn):
        ...
        v = s_x(u)
        u = s_y(v)
        #print((u.log()-u_prev.log()).abs().max().item()*eps)
